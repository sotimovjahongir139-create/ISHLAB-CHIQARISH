...
import { Refresh, Delete, Save, Edit, ContentCut } from '@mui/icons-material';
import { useState, useEffect, useCallback } from 'react';
import { useSnackbar } from 'notistack';
import * as svc from '../../services/kesish.service';
...
const KIcon = () => <ContentCut sx={{ fontSize: 30, color: '#EA580C' }} />;
...
const Kesish = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = await svc.getKesishRecords({ page: page + 1, limit: 20, startDate, endDate });
  ...
  const handleEditSave = async () => {
    ...
    setEditSaving(true);
    try {
      await svc.updateKesishRecord(editDialog.item.id, {
        date: editForm.date,
  ...
  const handleDelete = async () => {
    ...
    try {
      await svc.deleteKesishRecord(deleteDialog.item.id);
    ...
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2.5 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <KIcon />
          <Box>
            <Typography variant="h4">Kesish</Typography>
            <Typography variant="body2" color="text.secondary">Kesish bo'limi qaydlari</Typography>
          </Box>
        </Box>
      ...
      <Box sx={{ mb: 2 }}>
        <Chip
          ...
          sx={{ bgcolor: '#FFF7ED', color: '#EA580C', border: '1px solid #FDBA74', fontWeight: 600 }}
        />
      </Box>
          ...
          <Grid container spacing={1.5} alignItems="flex-end">
            ...
            <Grid item xs={6} sm="auto">
              <Button variant="contained" size="small"
                ...
                sx={{ bgcolor: '#EA580C', '&:hover': { bgcolor: '#C2410C' }, height: 40, whiteSpace: 'nowrap' }}>
                Saqlash
              </Button>
            </Grid>
          </Grid>
        ...
        {[['kunlik', 'Kunlik'], ['haftalik', 'Haftalik'], ['oylik', 'Oylik'], ['otgan_oy', "O'tgan oy"]].map(([p, label]) => (
          <Button key={p} size="small"
            ...
            sx={{
              ...
              ...(period === p ? { bgcolor: '#EA580C', '&:hover': { bgcolor: '#C2410C' }, borderColor: '#EA580C' } : {}),
            }}>
            {label}
          </Button>
        ))}
                ...
                <TableRow key={r.id} hover>
                  ...
                  <TableCell align="right">
                    <Chip label={r.producedQty ?? 0} size="small"
                      sx={{ bgcolor: '#FFF7ED', color: '#EA580C', fontWeight: 600 }} />
                  </TableCell>
                  <TableCell align="right">
                    {r.brakQty > 0
      ...
      <Dialog open={editDialog.open} onClose={() => setEditDialog({ open: false, item: null })} maxWidth="sm" fullWidth>
        <DialogTitle>Kesish yozuvini tahrirlash</DialogTitle>
        <DialogContent>
