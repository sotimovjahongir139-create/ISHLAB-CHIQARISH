...
const getRecords = async (query) => {
  ...
  const [data, total, agg] = await Promise.all([
    prisma.charxlashRecord.findMany({ where, include: INCLUDE, orderBy: { date: 'desc' }, skip, take: limit }),
    prisma.charxlashRecord.count({ where }),
    prisma.charxlashRecord.aggregate({ where, _sum: { producedQty: true } }),
  ]);
...
const createRecord = async (body) => {
  ...
  return prisma.charxlashRecord.create({
    data: {
...
const updateRecord = async (id, body) => {
  return prisma.charxlashRecord.update({
    where: { id },
    data: {
...
const deleteRecord = async (id) => {
  return prisma.charxlashRecord.update({ where: { id }, data: { isDeleted: true } });
};
...
const getStats = async (query) => {
  ...
  const [count, agg] = await Promise.all([
    prisma.charxlashRecord.count({ where }),
    prisma.charxlashRecord.aggregate({ where, _sum: { producedQty: true, plannedQty: true, brakQty: true } }),
  ]);
  return {
    recordCount: count,
